figure2 <- function(formatList){
    ...
    for(format in 1:length(formatList)){
        ...
        if(formatList[format] == 'pdf'){
            png(paste0(outfigdir,'Main_MS/pdf/Fig 2A.pdf'),width=8, height=6.4,units=units,res = res)
        }
        if(formatList[format] == 'png'){
            png(paste0(outfigdir,'Main_MS/png/Fig 2A.png'),width=8, height=6.4,units=units,res = res)
        }
        if(formatList[format] == 'eps'){
            postscript(paste0(outfigdir,'Main_MS/eps/Fig 2A.eps'),width=8, height=6.4,paper = "special")
        }

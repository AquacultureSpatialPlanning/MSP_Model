...
if Run0_load1_data_and_EFeval==0; %Takes several hours to run
    ...
    Policy_i_a_tmp = load('~/MSP_Model/Input/Data/U_C_obj_i.mat');
    Policy_i_a_tmp = struct2cell(Policy_i_a_tmp.U_C_obj_i);
    ...
    load('X_n_i_p','X_n_i_p')
    I = 1061;
    X_n_i_p_tmp = NaN(1061,7,4);
    X_n_i_p_tmp(:,:,1) = X_n_i_p.No_Development;X_n_i_p_tmp(:,:,2) = X_n_i_p.Develop_Mussel;X_n_i_p_tmp(:,:,3) = X_n_i_p.Develop_Finfish;X_n_i_p_tmp(:,:,1) = X_n_i_p.Develop_Kelp;
    X_n_i_p = X_n_i_p_tmp;
    ...
    save('EFPayoff_i_a_X_UC.mat','EFPayoff_i_a_M','EFPayoff_i_a_F','EFPayoff_i_a_K','EFPayoff_i_a_H','EFPayoff_i_a_V','EFPayoff_i_a_B','EFPayoff_i_a_D','-v7.3');
...
%     load Y_NPV_wrt_MSP
    load UC_Dynamic_Halibut %contains Y_NPV_wrt_MSP
    ...
    save('EFPayoff_a_X_wrt_DM_UC','EFPayoff_a_M_wrt_DM','EFPayoff_a_F_wrt_DM','EFPayoff_a_K_wrt_DM','EFPayoff_a_H_wrt_DM','EFPayoff_a_V_wrt_DM','EFPayoff_a_B_wrt_DM','EFPayoff_a_D_wrt_DM')
    ...
    % rows = i = 1....1061 sites
    % cols = a = 1....279936 unique alpha weighting scenarios
    Policy_i_a_tmp = load('~/MSP_Model/Input/Data/C_C_obj_i.mat');
    Policy_i_a_tmp = struct2cell(Policy_i_a_tmp.C_C_obj_i);
    Policy_i_a = NaN(1061,1062);
    for itor = 1:size(Policy_i_a_tmp,1)
       Policy_i_a(:,itor) = Policy_i_a_tmp{itor,1} + 1;
       tabulate(Policy_i_a(:,itor))
    end
    % Load other key variables
        % X_n_i_p = Matrix of sector unitless value in each patch given policy choices
            % dimension 1 = i = 1....1061 sites
    ...
    %Save values
    EFPayoff_i_a_X=EFPayoff_i_a_M;
    save('EFPayoff_i_a_X_CC.mat','EFPayoff_i_a_M','EFPayoff_i_a_F','EFPayoff_i_a_K','EFPayoff_i_a_H','EFPayoff_i_a_V','EFPayoff_i_a_B','EFPayoff_i_a_D','-v7.3');
...
%% scale payoffs so ranges ranges 0-1 relative to max and min domain-wide payoff (note, this is not the same as the scaling done in the tradeoff analysis)
    ...
    %overwrite halibut with dynamic response
%     load Y_NPV_wrt_MSP
    load CC_Dynamic_Halibut %contains Y_NPV_wrt_MSP
    EFPayoff_a_H=Y_NPV_wrt_MSP_unique';
    EFPayoff_a_V=nansum(EFPayoff_i_a_V,1);
    EFPayoff_a_B=nansum(EFPayoff_i_a_B,1);
    EFPayoff_a_D=nansum(EFPayoff_i_a_D,1);
    ...
    %save results
    EFPayoff_a_X_wrt_DM=EFPayoff_a_M_wrt_DM;
    save('EFPayoff_a_X_wrt_DM_CC','EFPayoff_a_M_wrt_DM','EFPayoff_a_F_wrt_DM','EFPayoff_a_K_wrt_DM','EFPayoff_a_H_wrt_DM','EFPayoff_a_V_wrt_DM','EFPayoff_a_B_wrt_DM','EFPayoff_a_D_wrt_DM')
elseif Run0_load1_data_and_EFeval==1
    disp('Loading EFPayoff_a_X_wrt_DM')
    load EFPayoff_a_X_wrt_DM_UC
    load EFPayoff_a_X_wrt_DM_CC
end

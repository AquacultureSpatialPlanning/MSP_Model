...
disp('Calculate adult movement')
    ...
    % Marichev, O. I. "Funktionen vom hypergeometrischen Typ und einige Anwendungen auf Integral- und Differentialgleichungen." Ph.D. dissertation. Jena, Germany: Friedrich-Schiller-Universit�t, p. 266, 1990.
...
axis tight
axis square
print(strcat(output_figure_dir,'m_i_j'),'-djpeg')
...
print(strcat(output_figure_dir,'Adult_movement_wrt_pairwisepatchdistance'),'-djpeg')
...
box off
title('i = source; j = destination')
print(strcat(output_figure_dir,'Adult_movement_wrt_pairwisepatchhabitatsuitability'),'-djpeg')
...
% xlim([0 10])
title('i = source; j = destination')
print(strcat(output_figure_dir,'Adult_movement_wrt_pairwisepatchhabitatsuitabilityfcn'),'-djpeg')

% data_save_adult_movement=mij;
% save data_save_adult_movement
if halibut_max_depth==-183
    save(strcat(output_data_dir,'mij_183depthlimit'),'mij')
elseif halibut_max_depth==-90
    save(strcat(output_data_dir,'mij_90depthlimit'),'mij')
else
    save(strcat(output_data_dir,'mij'),'mij')
end

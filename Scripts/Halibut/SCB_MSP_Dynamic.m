...
% if run_dyn_eval_MSPsolns_0N_1Y==1
...
for pt=1:length(Policy_i_a_trans_C(:,1))
    ...
    if isempty(intersect(pt,ipt_counter))==0
        ...
        save(strcat(DirPart,'/Output/Data/DynHalEval_wrt_MSP.mat'),'Y_NPV_wrt_MSP','Y_NPV_wrt_MSP_unique','pt') %save data generated so far
    end
...
%Fill in unique policy results where redundant
for p=1:length(Policy_i_a(1,:))
    Y_NPV_wrt_MSP(p)=Y_NPV_wrt_MSP_unique(Policy_i_a_trans_IC(p));
end

    save(strcat(DirPart,'/Output/Data/DynHalEval_wrt_MSP.mat'),'Y_NPV_wrt_MSP','Y_NPV_wrt_MSP_unique','pt') %save data generated so far
    disp(['Took = ',(num2str(toc/60/60/24)),' days'])
% end
exit;

...
%% First distribute effort proportional to virgin fish biomass, run non-fleet core, and use that as the initial conditions and for setting PPUE_guess
% if exist('x0_PPUE_initial')==0
if x0_PPUE_initial>=0
    x0_PPUE=x0_PPUE_initial;
else
    if Fsum==0
        x0_PPUE=0;
    else
        Ei=zeros(numpatches,1);
...
% tol_simulation=1e-10; %proportion difference in biomass allowed. 1e-6 default
...
%     Yield - TIME SERIES DATA  
    ...
    Bijy(:,:,y)=Bij; %biomass at end of year - TIME SERIES DATA

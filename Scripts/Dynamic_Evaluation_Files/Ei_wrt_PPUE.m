...
function [out]=Ei_wrt_PPUE(x0_PPUE,habitat_area_i,theta,price,Bi_legal,delta,gammatmp,distance_to_port_for_each_soft_depth_patch,SQ_1fishable_0notfishable_for_each_soft_depth_patch)
...
% global Bi_legal price delta theta Fsum habitat_area_i gamma distance_to_port_for_each_soft_depth_patch SQ_1fishable_0notfishable_for_each_soft_depth_patch qi
 load('Tuner_parameters','habitat_area_i','theta','price','gamma','Bi_legal','delta','distance_to_port_for_each_soft_depth_patch')
 gammatmp=gamma;
...
%     out=Ei_vector_fleet;
    ...
    %ACCOUNTING FOR SQ FISH EXCLUSION PATHCES
     ...
     Z=(x0_PPUE+habitat_area_i_fishable.*theta.*(1+gammatmp.*distance_to_port_for_each_soft_depth_patch_fishable))./(price.*Bi_legal_fishable);
    lambert_z=-exp(-1./Z)./Z;
    %wapr will break if lambert_z is out of range (i.e., <-exp(-1)); so set
    %those values to the edge of the range = -exp(-1)
    lambert_z(lambert_z<-exp(-1))=-exp(-1);

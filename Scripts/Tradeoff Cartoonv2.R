Tradeoff.cartoon <- function(){
  ...
  cex = .08
  tolerence=10
  x=100*Master.matrix.max$Finfish
  y=100*Master.matrix.max$Viewshed
  color.vector=color.vector.max
  x.MSP=x[color.vector=='coral']
  y.MSP=y[color.vector=='coral']
  # points(x.MSP,y.MSP, pch = 16, col = alpha("coral",1/50),cex = cex)
  ...
  starting.point=min(x.EF)
  ending.point.U=max(x.U)
  ending.point.U=max(x.S)
  range.U=seq(from=starting.point,to=ending.point.U,by=100)
  range.S=seq(from=starting.point,to=ending.point.U,by=100)
  for(itor in 1:length(range.U)){
  ...
  num.lines.U=seq(from=1,to=max(I.U.store),by=15)
  num.lines.S=seq(from=1,to=max(I.S.store),by=60)
  ...
  #   print(paste0('Coordinate Value Y is ',U.Value.y,' For Iteration = ',itor))
    ...
    if(itor==63){
      Arrows(x0=MSP.Value.x,y0=88.726,x1=MSP.Value.x,y1=MSP.Value.y, col="mediumorchid1",lty=1,lwd=lwd, arr.type = arr.type, arr.adj = arr.adj, arr.length = arr.length)
    }else{
      Arrows(x0=MSP.Value.x,y0=U.Value.y,x1=MSP.Value.x,y1=MSP.Value.y, col="mediumorchid1",lty=1,lwd=lwd, arr.type = arr.type, arr.adj = arr.adj, arr.length = arr.length)
    }
  }
  ...
  for(itor in 1:length(num.lines.S)){
    ...
    Arrows(x0=MSP.Value.x,y0=S.Value.y,x1=MSP.Value.x,y1=MSP.Value.y, col="coral1",lty=1,lwd=lwd, arr.type = arr.type, arr.adj = arr.adj, arr.length = arr.length)
  }
  p_cartoon<-recordPlot()
  dev.off()
  return(p_cartoon)
}
...
# U.arrow.tmp=cbind(sort(x.U),y.U[order(x.U)])
# S.arrow.tmp=cbind(sort(x.S),y.S[order(x.S)])
# U.arrow=U.arrow.tmp[994,]
# x0.U1=U.arrow[1]
# y0.U1=U.arrow[2]
# x1.U1=x0.U1
# y1.U1=y.EF[which(min(abs(x0.U1-x.EF))==abs(x0.U1-x.EF))]
# x0.U2=x0.U1
# y0.U2=y0.U1
# x1.U2=x.EF[which(min(abs(y0.U2-y.EF))==abs(y0.U2-y.EF))]
# y1.U2=y1.U1
# Arrows(x0=x0.U1,y0=y0.U1,x1=x1.U1,y1=y1.U1, col='purple',lty=1,lwd=lwd, arr.type = arr.type, arr.adj = arr.adj, arr.length = arr.length)
# Arrows(x0=x0.U2,y0=y0.U2,x1=x1.U2,y1=y0.U2, col='purple',lty=1,lwd=lwd, arr.type = arr.type, arr.adj = arr.adj, arr.length = arr.length)
#
#
# S.arrow=S.arrow.tmp[737,]
# x0.S1=S.arrow[1]
# y0.S1=S.arrow[2]
# x1.S1=x0.S1
# y1.S1=y.EF[which(min(abs(x0.S1-x.EF))==abs(x0.S1-x.EF))]
# x0.S2=x0.S1
# y0.S2=y0.S1
# x1.S2=x.EF[which(min(abs(y0.S2-y.EF))==abs(y0.S2-y.EF))]
# y1.S2=y1.S1
# Arrows(x0=x0.S1,y0=y0.S1,x1=x1.S1,y1=y1.S1,col='green',lty=1,lwd=lwd, arr.type = arr.type, arr.adj = arr.adj, arr.length = arr.length)
# Arrows(x0=x0.S2,y0=y0.S2,x1=x1.S2,y1=y0.S2,col='green',lty=1,lwd=lwd, arr.type = arr.type, arr.adj = arr.adj, arr.length = arr.length)
